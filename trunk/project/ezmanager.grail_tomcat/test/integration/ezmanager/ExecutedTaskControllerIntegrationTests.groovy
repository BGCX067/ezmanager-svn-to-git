...
import ezmanager.ExecutedTaskSearchCommand;
import ezmanager.Project;
import ezmanager.Task;
import ezmanager.UserIdentity;
import ezmanager.domain.*

class ExecutedTaskControllerIntegrationTests extends ControllerUnitTestCase {
	...
	void testSearch() {
		ExecutedTask.withTransaction() { task ->
			// Status
			...
			ExecutedTask et = new ExecutedTask(totalHours:253, amount:2000, startDate:startDateExecutedTaskRef, endDate:endDateExecutedTaskRef, task:taskRef, executedByUser:userRef)
			et.save()

			def filter = new ExecutedTaskSearchCommand(totalHoursMin:252, totalHoursMax:254, amountMin:1999, amountMax:2001, startDate:startDateExecutedTaskRef-1, endDate:endDateExecutedTaskRef+1)
